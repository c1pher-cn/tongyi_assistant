...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> None:
    ...
    dashscope.api_key = data[CONF_API_KEY]
    #openai.api_key = data[CONF_API_KEY]

    messages = [{'role': 'system', 'content': 'You are a helpful assistant.'},
                {'role': 'user', 'content': 'Hi'}]

    #await hass.async_add_executor_job(partial(dashscope.Generation.call(
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        try:
            ...
            return self.async_create_entry(title="Tongyi Assistant01", data=user_input)
